...
namespace algebra {
    const int inf = 1e9;
    const int magic = 50;  // threshold for sizes to run the naive algo
    ...
    struct poly {
      ...
      poly Sqrt(site_t n) {
        return (log(n) * poly(T(2).inv())).exp(n);
      }
    ...
    template <typename T> 
    poly<T> Kmul(const vector<T>& x) {
      return kmul(x.begin(), x.end());
    }
...
int main() {
    ...
    while (cin >> M >> N >> Q) {
        ...
        if (Q > 10) {
        // if (false) {
            auto pinv = kmul(inv.begin(), inv.end());
            // show(v, v.size());
            // show(inv, inv.size());
            // show(pinv.a, pinv.a.size());
            // for (auto i : inv) {
                // cout << "# " << i << ' ' << pinv.eval(i) << endl;
            // }
            auto ainv = pinv.inv(M);
            auto m = pinv*ainv;
            // show(m.a, m.a.size());
            if (N % 2) {
                for (auto &a : ainv.a) {
            ...
            // out(ainv.a.size());
            // show(ainv.a, ainv.a.size());
            rep(i, Q) cout << ainv.a[q[i]-1] << endl;
        } else {
            auto pinv = kmul(inv.begin(), inv.end()).deriv();
            auto fx = pinv.eval(inv);
            // show(fx, fx.size());
            for (auto x : q) {
                base ans = 0;
                rep(i, N) {
                    ans += bpow(v[i], x) * fx[i].inv();
                }
                if (N%2 == 0) ans = -ans;
                // out(ans);
                cout << ans << endl;
